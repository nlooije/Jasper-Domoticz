...
class DomoticzPlugin(plugin.SpeechHandlerPlugin):

    def __init__(self, *args, **kwargs):
        super(DomoticzPlugin, self).__init__(*args, **kwargs)
        self.server, self.credentials = self.get_profile_info()
    ...
    def get_profile_info(self):
        """
            ...
            Returns:
            Server url and credentials for Domoticz server
        """
        profile = self.profile['domoticz']
    ...
    def get_phrases(self):
        return [self.gettext(kw) for kw in KEYWORDS]
    ...
    def handle(self, text, mic):
        ...
        def send_request(url):
            ...
            response = requests.get(self.server + url, 
                                    ...
                                    verify=False) #TO-DO: This might a security issue
        ...
        def handle_scene(scene):
            """
                Contains the logic to handle scenes and groups. 
                Scenes can only be activated but devices in the scene can have mixed states. 
                Groups have an on/off state but all devices within the group have the same state.
